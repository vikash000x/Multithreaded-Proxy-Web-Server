...
//which element we want to store in cache
...
//ll se find karenge wo url
cache_element* find(char* url);
//wo url, uska data aur size teeno add karenge
int add_cache_element(char* data,int size,char* url);
//ll se hatayenge vi access jo ban jata hai cache bhar jayegi to
void remove_cache_element();

//agr jyada log jo hamara client hai ek hi time pe multiple url hit kr rhe hai to savi ka alag 
//socket banega jo cache me url store krne ke baad jo vi response aayega wo store ho jayega uske socket me
// aur sare socket proxy server se connect honge because of multithreading concept
// client----> google.com----->| socket(res) |----------------->proxy server
//jitne client utne thread
...
//t1, t2 if both thread are likely to connect at same time with LRU cache because it is shared resource race condition can develop
//that is why use lock
...
void* thread_fn(void* socketNew)
{
	...
	// req aayi hai proxy tak ab ise cache me search vi krna hai
